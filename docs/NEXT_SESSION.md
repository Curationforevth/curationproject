...
## 🔲 다음 후보 (Eden 판단)
1. **폰 체감 확인**: '읽었어요' 정상 + 서재 상태 배지/버튼 + 홈 당겨서 새로고침 시
   새 큐레이션 제목들(리라이트 반영 — hour 캐시라 새로고침 필요).
2. top_n 700 체감이 무거우면 300/500 하향(config 한 줄, 재검증 하네스 있음).
3. 큐레이션 2차 고도화: 리라이트 문구가 아직 다소 밋밋("~의 세계" 류) — 톤 프롬프트 개선
   또는 취향 기반 동적 제목("당신이 좋아한 기묘함" 류, Spotify 패턴)은 별도 설계.
