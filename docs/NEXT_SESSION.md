# 다음 세션 핸드오프 (2026-07-02 #13 — 세션 종료)

> 이번 세션 = **PR#35~#46, 12개 전부 머지·배포·prod 검증** (서버+앱+마이그레이션):
...
## 🔲 다음 후보 (Eden 판단)
1. **폰 체감 확인(최신 빌드 설치됨)**: '읽었어요' 전이 / 서재 상태 배지·버튼 / 홈
   새로고침마다 큐레이션 2칸+화제의 책 전부 바뀌는지 / 커버·중복·저자 표기.
2. **배포 시간 단축 설계**: 이미지에서 index.pkl 256MB 분리(시작 시 다운로드) →
   배포 8.5~12분 → 2~3분대. 오늘 7회 배포로 체감 큰 페인포인트.
3. top_n 700 체감이 무거우면 300/500 하향(config 한 줄, verify 하네스 재검증).
4. 큐레이션 2차 고도화: 리라이트 톤 개선 / 취향 기반 동적 제목(Spotify 패턴) /
   tier2+ 전용 테마 공급(클러스터 기반 심화 — 슬롯은 이미 우선 노출 준비됨).
5. stage1 랭킹 미스 케이스(recall 고정 유저) 원인 분석 — 취향 산발 유저 품질 레버.
6. 섹션 간 '판본' 중복(제목+저자 기준) / 커버 http URL 3권(https 가용 확인됨) 정리.
7. 미정제 keyword 잔여 3건은 주간 워크플로가 자동 재시도(방치 OK).
