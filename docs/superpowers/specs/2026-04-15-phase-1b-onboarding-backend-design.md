...
| `books` | `l1 TEXT` (generated), `l2 TEXT` (generated), `idx_books_l1_l2` 인덱스 |
...
| impression (현재 non-batch, 평균 30 imp/call × 1,500 req × 500B) | 22.5 | 675 |
...
```bash
# 0. 환경 변수 (test user JWT 발급: Supabase Dashboard → Authentication → Users → test 계정 생성 → Sign in 후 access_token 복사)
    ...
    steps:
      ...
      - run: |
          psql $DB_URL -c "SELECT jobname FROM cron.job" | \
            grep -E "refresh-curation-cache|aggregate-co-occurrence|refresh-user-top-taste|refresh-fallback-curation|deactivate-curations|check-stage-transition|cleanup-user-curation-history|cleanup-home-section-cache"
      - run: |
...
- feature flag 시스템
- cluster 개수 조정 (수동 파라미터, 책 수 증가에 따라)
- plpgsql test 프레임워크
- Render staging 환경
