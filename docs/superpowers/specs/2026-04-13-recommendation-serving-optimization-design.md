...
- 이미 computing=True면 새 task는 skip (마지막 상태로 나중에 자연스럽게 재계산됨).
...
async def get_recommendations(user_id, request, limit, background_tasks):
    ...
    background_tasks.add_task(
        save_cache, user_id, top_recs, input_hash, ...)
...
**주의**: 위 수치는 로컬 Mac 기준. Render free tier (0.5 vCPU shared)에서는 2~5x 느릴 수 있음.
